...
module.exports.run = async (opts = {}) => {
  if (process.argv.includes('--update-helper')) {
  ...
  if (options.TIMER > 0 && !WATCHTOWER_UPDATE_PENDING) {
    setTimeout(async () => {
...
module.exports.checkPendingUpdate = async () => {
  const { WATCHTOWER_UPDATE_PENDING } = app.config().system;
  if (WATCHTOWER_UPDATE_PENDING) {
    ...
    logger.log('watchtower-helper container created');
  }
};
...
module.exports.autoUpdate = async () => {
  ...
  if (!info.updateReady) {
    logger.log('watchtower not running - skipping update');
    return;
  }
  ...
  const createOptions = {
    name: info.containerName, ...data.Config, HostConfig: data.HostConfig, NetworkingConfig: data.NetworkingConfig,
  };
  await container.stop(info.containerId);
  await container.remove();
  const createdContainer = await docker.createContainer(createOptions);
  await createdContainer.start();
