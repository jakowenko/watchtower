...
module.exports.restart = async () => {
  ...
  for (let i = 0; i < images.length; i += 1) {
    ...
    // logger.log(createOptions);

    logger.log(`${createOptions.name}`);
    await container.stop(container.Id);
    // logger.log(`stopped ${createOptions.name}`);
    await container.remove();
    // logger.log(`removed ${createOptions.name}`);
    const createdContainer = await docker.createContainer(createOptions);
    // logger.log(`created ${createOptions.name}`);
    await createdContainer.start();
    // logger.log(`started ${createOptions.name}`);
  }
