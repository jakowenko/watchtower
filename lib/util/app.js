...
module.exports.init = (opts = {}) => {
  ...
  const options = {
    ...
    NOTIFY_EMAIL_IGNORE_TLS: env.NOTIFY_EMAIL_IGNORE_TLS === 'true',
    ...
    AUTO_UPDATE_WATCHTOWER: env.AUTO_UPDATE_WATCHTOWER !== 'false',
  ...
  for (const key in opts) {
    ...
    } else if ({}.hasOwnProperty.call(opts, key) && key === 'docker' && opts[key]) {
      system.DOCKER = true;
    }
  }
  ...
  app.save(system, options);
  return { system, options };
};

module.exports.config = () => JSON.parse(fs.readFileSync('data/app.json'));

module.exports.system = () => JSON.parse(fs.readFileSync('data/app.json')).system;

module.exports.options = () => JSON.parse(fs.readFileSync('data/app.json')).options;
...
module.exports.update = (values = {}) => {
  ...
  app.save(config.system, config.options);
};
